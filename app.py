...
def generate_weights(n):
    ...
    if n == 1:
        ...
        for i in range(1, 10):
            weights.append({0: i/10, 1: (10-i)/10})
    elif n == 3:
        for i in range(1, 9):
            for j in range(1, 9-i):
                k = 10 - i - j
                if k > 0:
                    weights.append({0: i/10, 1: j/10, 2: k/10})
    elif n == 4:
        for i in range(1, 8):
...
with st.sidebar:
    ...
    with col_a:
        st.markdown("**TRYJPY**")
        try_min_pct = st.slider("最低比率 (%)", 0, 50, 5, step=5, key="try_min")     # おすすめ初期値
        try_max_pct = st.slider("最高比率 (%)", try_min_pct, 100, 35, step=5, key="try_max")
    
    with col_b:
        st.markdown("**MXNJPY**")
        mxn_min_pct = st.slider("最低比率 (%)", 0, 50, 5, step=5, key="mxn_min")     # おすすめ初期値
        mxn_max_pct = st.slider("最高比率 (%)", mxn_min_pct, 100, 40, step=5, key="mxn_max")
    
    with col_c:
        st.markdown("**ZARJPY**")
        zar_min_pct = st.slider("最低比率 (%)", 0, 50, 0, step=5, key="zar_min")     # おすすめ初期値
        zar_max_pct = st.slider("最高比率 (%)", zar_min_pct, 100, 35, step=5, key="zar_max")
    
    st.caption("その他の通貨保有比率の制限")
    other_limit = st.slider("🌍 TRY/MXN/ZAR以外の最大比率制限 (%)", 10, 100, default_other_limit, step=10)
...
if is_demo_mode:
    st.warning("🚧 現在は**デモモード**です。選択できる通貨ペアが制限されています。")
    ...
    with st.spinner("⏳ データ取得＆最適化計算中..."):
        ...
        if df_full is None or df_full.empty:
            ...
            if "USDJPY" not in df_calc.columns:
                st.error("❌ USDJPYのデータ取得に失敗しました。再試行してください。")
                st.stop()
            ...
            for col in df_calc.columns:
                if col == "USDJPY":
                    ...
                    betas[col] = calculate_beta(df_calc[col], df_calc["USDJPY"])
                ...
                for combo in itertools.combinations(buy_candidates, size):
                    ...
                    for wp in weights_list:
                        ...
                        for ccy, weight in pattern.items():
                            ...
                            if ccy == "TRYJPY":
                                if w_pct < try_min_pct or w_pct > try_max_pct:
                                    is_valid_weight = False
                                    break
                            elif ccy == "MXNJPY":
                                if w_pct < mxn_min_pct or w_pct > mxn_max_pct:
                                    is_valid_weight = False
                                    break
                            elif ccy == "ZARJPY":
                                if w_pct < zar_min_pct or w_pct > zar_max_pct:
                                    is_valid_weight = False
                                    break
                            else:
                                # その他の通貨（PLNJPY, CZKJPYなど）
                                if w_pct > other_limit:
                                    is_valid_weight = False
                                    break
            ...
            if final_best is None:
                ...
                if total_combinations > 0 and rejected_by_ratio == total_combinations:
                    st.warning(
                        f"💡 **原因診断:** 通貨保有比率の制限が厳しすぎます。\n"
                        f"・TRY: {try_min_pct}%〜{try_max_pct}%\n"
                        f"・MXN: {mxn_min_pct}%〜{mxn_max_pct}%\n"
                        f"・ZAR: {zar_min_pct}%〜{zar_max_pct}%\n"
                        f"・その他: 〜{other_limit}%\n"
                        "これらの条件を緩めて再計算してください。"
                    )
